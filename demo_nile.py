...
logL,fvar = ssm.kalman_int(4,n,y,mis,anymis,allmis,llm)
...
plt.title('Filtered state variance')
plt.ylim([5000,17500])
...
ax4 = plt.subplot(224)
plt.plot(time,F)
plt.title('Prediction error variance')
...
#-- State smoothing --#
alphahat,V,r,N    = ssm.statesmo_int(1,n,y,mis,anymis,allmis,llm)
...
plt.plot(time,alphahat-1.645*np.sqrt(V),'g:',label='90% conf. -')
plt.title('Smoothed state')
plt.ylim([450,1400])
plt.legend()

ax2 = plt.subplot(222)
plt.plot(time,V)
plt.title('Smoothed state variance')
plt.ylim([2300,4100])
ax3 = plt.subplot(223)
plt.plot(time,r)
plt.title('Smoothing cumulant')
plt.ylim([-0.036,0.024])
ax4 = plt.subplot(224)
plt.plot(time,N)
plt.title('Smoothing variance cumulant')
plt.ylim([0,0.000105])
...
epshat,etahat,epsvarhat,etavarhat = ssm.disturbsmo_int(1,n,y,mis,anymis,allmis,llm)
# Reshape output for plotting
...
plt.title('Observation error variance')
plt.ylim([2300,4100])
ax3 = plt.subplot(223)
...
plt.title('State error variance')
plt.ylim([1225,1475])
...
for ii in range(NN):
    alphatilde,epstilde,etatilde,alphaplus = ssm.simsmo_int(1,n,y,mis,anymis,allmis,llm)
...
anymis  = np.any(mis,0)
allmis  = np.all(mis,0)
amis,Pmis,dmis   = ssm.kalman_int(1,n,ymis,mis,anymis,allmis,llm)[:3]
alphahatmis,Vmis = ssm.statesmo_int(1,n,ymis,mis,anymis,allmis,llm)[:2]
amis[:,:dmis+1]   = np.nan
Pmis[:,:,:dmis+1] = np.nan
amis        = amis.squeeze()
Pmis        = Pmis.squeeze()
...
plt.title('Filtered state (extrapolation)')
plt.ylim([450,1400])
plt.legend()
ax2 = plt.subplot(222)
...
plt.title('Smoothed state (interpolation)')
plt.ylim([450,1400])
plt.legend()
ax4 = plt.subplot(224)
plt.plot(time, Vmis)
...
anymis  = np.any(mis,0)
allmis  = np.all(mis,0)
aforc,Pforc,dforc,vforc,invFforc = ssm.kalman_int(1,n,yforc,mis,anymis,allmis,llm)
# Build ndarrays from list output
...
plt.title('Observation forecast')
plt.xlim([1868,2026])
plt.ylim([700,1200])
ax4 = plt.subplot(224)
plt.plot(time+range(1972,2022), Fforc)
plt.title('Observation forecast variance')
plt.xlim([1868,2026])
plt.ylim([20000,96000])
